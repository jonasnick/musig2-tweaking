...
\jnote{The attack probably doesn't require such large $L$ but can work with multiple smaller multisets of keys? NO if we'd select between different $L$ then $b$ would change between the sessions which changes target $R^*$.}
 ...
 \begin{tcolorbox}[boxsep=-1mm]
  \begin{pchstack}[center]
   \begin{pcvstack}
    ...
    \pcvspace
    \procedure{$\sign(\textcolor{red}{\sk_1, t})$}{%
%       (p,\GG,g) \defeq \param \\
      \pclinecomment{Local signer has index $1$.} \\
      \pcfor j \defeq 1 \ldots \nu \pcdo \\
        \t r_{1,j} \sample \ZZ_p \pcsc R_{1,j} \defeq g^{r_{1,j}} \\
      \msg_1 \defeq (R_{1,1},\ldots,R_{1,\nu}) \\
      \state_1 \defeq (r_{1,1},\ldots,r_{1,\nu}) \\
      \textcolor{red}{\Tsesh(r_{1,1},\ldots,r_{1,\nu}) = \textcolor{red}{\sk_1 + t \bmod p}}\\
      \pcreturn (\msg_1,\state_1)
    }
   \end{pcvstack}
   \iffull \pchspace[5em] \else \pchspace[0.7em] \fi
   ...
   \procedure{$\signagg(\msg_1,\ldots,\msg_n)$}{%
    ...
    \pcvspace
    \procedure{$\sign'(\state_1,\msg,m,(\pk_2,\ldots,\pk_n))$}{%
      \pclinecomment{$\sign'$ must be called at most once per $\state_1$.} \\
      (r_{1,1},\ldots,r_{1,\nu}) \defeq \state_1 \\
      \textcolor{red}{x_1 \defeq \Tsesh(r_{1,1}, \ldots, r_{1,\nu})}\\
      X_1 \defeq g^{x_1} \\
      (X_2, \ldots, X_n) \defeq (\pk_2, \ldots, \pk_n)\\
      L \defeq \{X_1,\ldots,X_n\} \\
      a_1 \defeq \musigcoef(L,X_1)\\
